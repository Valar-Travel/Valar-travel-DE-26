...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  if (!villa) {
    return {
      title: "Villa Not Found | Valar Travel",
      description:
        "The requested luxury villa could not be found. Browse our collection of handpicked Caribbean villas.",
    }
  }

  const bedroomText = villa.bedrooms ? `${villa.bedrooms} bedroom` : ""
  const locationText = villa.location || "Caribbean"
  const priceText = villa.price_per_night ? `from $${villa.price_per_night}/night` : ""
  const amenitiesText = villa.amenities?.slice(0, 3).join(", ") || ""

  const description = `${villa.name} - ${bedroomText ? `Luxurious ${bedroomText} villa in ${locationText}` : `Exclusive luxury villa in ${locationText}`}${priceText ? ` ${priceText}` : ""}. ${amenitiesText ? `Features: ${amenitiesText}.` : ""} Book your Caribbean dream escape with Valar Travel's personalized concierge service.`

  return {
    title: `${villa.name} | Luxury Villa in ${locationText} | Valar Travel`,
        ...
        ? [
            {
              ...
              width: 1200,
              height: 630,
              alt: `${villa.name} - Luxury villa exterior view in ${locationText}`,
            },
          ]
        : [],
      locale: "en_US",
...
export default async function VillaDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  let amenities = villa.amenities || []
  if (Array.isArray(amenities) && amenities.length === 0 && villa.description) {
    amenities = extractAmenitiesFromDescription(villa.description)
  }
  ...
  const villaImages =
    villa.images && Array.isArray(villa.images) && villa.images.length > 0
      ? villa.images
      : [
          `/placeholder.svg?height=600&width=800&query=${encodeURIComponent(villa.name + " luxury villa exterior")}`,
          `/placeholder.svg?height=400&width=600&query=${encodeURIComponent(villa.name + " bedroom suite")}`,
          `/placeholder.svg?height=400&width=600&query=${encodeURIComponent(villa.name + " modern kitchen")}`,
          `/placeholder.svg?height=400&width=600&query=${encodeURIComponent(villa.name + " pool terrace")}`,
        ]

  const pricePerNight = villa.price_per_night || villa.price || 0
  const currency = villa.currency || "USD"

  const bedrooms = villa.bedrooms || null
  const bathrooms = villa.bathrooms || null
  const maxGuests = villa.max_guests || villa.guests || null

  // Parse description to extract clean sections and remove duplicates
  const parsedDescription = parsePropertyDescription(villa.description)
        ...
        <ol className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          {villa.location && (
            <>
              ...
              <li>
                <Link
                  href={`/destinations/${villa.location.toLowerCase().replace(/\s+/g, "-")}`}
          ...
          <li>/</li>
          <li className="text-foreground font-medium truncate max-w-[200px]">{villa.name}</li>
        </ol>
      </nav>
          ...
          <div className="lg:col-span-2 space-y-6 md:space-y-10">
            ...
            <div>
              <div className="flex items-center gap-2 text-xs md:text-sm text-muted-foreground mb-2">
                ...
                <span className="uppercase tracking-widest font-medium">{villa.location || "Caribbean"}</span>
              </div>
              ...
              <Card className="shadow-xl border-0 overflow-hidden">
                ...
                <CardContent className="p-5 space-y-5">
                  ...
                  <div className="space-y-2.5">
                    ...
                    >
                      <a
                        href={`${WHATSAPP_LINK}?text=${encodeURIComponent(`Hi Sarah, I'm interested in ${villa.name}. Can you help me with availability?`)}`}
                        target="_blank"
                  ...
                  <div className="pt-4 border-t">
                    ...
                    <div className="space-y-2">
                      ...
                      <a
                        href={`mailto:${CONTACT_EMAIL}?subject=${encodeURIComponent(`Inquiry: ${villa.name}`)}`}
                        className="flex items-center justify-center gap-2 p-2.5 rounded-lg bg-slate-50 hover:bg-emerald-50 transition-colors text-sm"
                      >
