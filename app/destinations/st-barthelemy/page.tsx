...
import { MapPin, Star, Bed, Bath, Users, Sparkles, Wine, Anchor, ShoppingBag } from "lucide-react"
...
const STBARTHS_SOURCE_URLS = [
  "stbarth.com",
  "stbarthsproperties.com",
  "wimco.com",
  "sibarth.com",
  "edenluxuryhomes.com",
]
...
async function getStBarthsVillas() {
  ...
  const { data, error } = await supabase
    ...
    .select("*")
    .or(
      `location.ilike.%st. barth%,location.ilike.%saint barth%,location.ilike.%st barth%,location.ilike.%st. barthelemy%,${STBARTHS_SOURCE_URLS.map((url) => `source_url.ilike.%${url}%`).join(",")}`,
    )
